...
import io.openmessaging.logging.Level;
import io.openmessaging.logging.Logger;
import io.openmessaging.logging.LoggerAdapter;
import java.io.File;
...
public class JdkLoggerAdapter implements LoggerAdapter {
	...
	public JdkLoggerAdapter() {
		try {
			InputStream in = Thread.currentThread()
									...
									.getResourceAsStream("logging-jdk.properties");
			if (in != null) {
				LogManager.getLogManager().readConfiguration(in);
			} else {
				System.err.println("No such logging-default.properties in classpath for jdk logging config!");
			}
		} catch (Throwable t) {
			System.err.println("Failed to load logging-default.properties in classpath for jdk logging config, cause: " + t.getMessage());
		}
		try {
	...
	static java.util.logging.Level toJdkLevel(Level level) {
		if (level == Level.ALL)
	...
	static Level fromJdkLevel(java.util.logging.Level level) {
